...
public class RicercaService implements IRicercaService {
	...
	/* (non-Javadoc)
	 * @see it.pbc.chiloripara.services.IRicercaService#getArtigianiByCategoria(java.lang.Long)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see it.pbc.chiloripara.services.IRicercaService#getArtigianiByCatAndCoord(java.lang.Long, float, float)
	 */
	@Override
	public List<ArtigianoDistanza> getArtigianiByCatAndCoord(Long catId, float lat, float lng) {
		...
		if (artigiani != null && artigiani.size() > 0) {
			...
			for (int i = 0; i < artigiani.size(); i++) {
				...
				artDist.setDistanza(distCalc.calculateDistance(lat, lng, artDist.getArt().getLat(), artDist.getArt().getLng()));
				result.add(artDist);
			}
	...
	@Override
	public List<ArtigianoDistanza> getArtigianiByCatAndAddress(Long catId, String address) throws Exception {
		// TODO Auto-generated method stub
		HashMap<String, Float> coordinate;
		List<ArtigianoDistanza> result;
		try {
		    coordinate = googleService.getCoordinate(address);
		    if(coordinate.get("LAT")==null || coordinate.get("LAT").equals(""))
		    	throw new Exception("Errore recupero coordinate");
		   result= this.getArtigianiByCatAndCoord(catId, coordinate.get("LAT"), coordinate.get("LNG"));
